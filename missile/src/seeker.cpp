...
//void Missile::seeker(Packet* combus, int num_vehicles, double int_step, double* arr, int length)
//{
//	Matrix STEL(3, 1);
//
//	Matrix VTEL(3, 1);
//
//	STEL.build_vec3(arr[0], arr[1], arr[2]);  // 坐标
//
//	double psial = arr[3] * RAD;
//	double thtal = arr[4] * RAD;
//
//	VTEL.cart_from_pol(arr[5], psial, thtal); // 
//
...
void Missile::seeker(int num_vehicles,double int_step,double* arr, int length, float start_time)
{
	//local variables
	...
	missile[500].gets(maut);
	double psial = arr[3] * RAD;
	double thtal = arr[4] * RAD;
	
	//STEL.build_vec3(0, 0, -10000);

	STEL.build_vec3(arr[0], arr[1], arr[2]);
	VTEL.cart_from_pol(arr[5], psial, thtal);
