...
class MemoryAllocator{
    ...
    int ptr_3_num = 0;
    int ptr_9_num = 0;
    const int ptr_num = 8096;
	double** ptr_3_list = new double* [ptr_num];
    double** ptr_9_list = new double* [ptr_num];

    MemoryAllocator(){
        cout<<"MemoryAllocator()"<<endl;
        for(int i=0; i<ptr_num; i++){
            ptr_3_list[i] = new double[3];
            ptr_9_list[i] = new double[9];
        }
    };
    ~MemoryAllocator(){
        cout<<"~MemoryAllocator()"<<endl;
    ...
    double* allocate_3(){
//        cout<<"=== allocate_3 ===   ptr_3_num="<<ptr_3_num<<endl;
        if (ptr_3_num == ptr_num - 1){
            double* ptr = new double[3];
//            cout<<"=== new3 [] "<<ptr<<endl;
            return ptr;
        }
        else{
//            cout<<"=== allocate3 [] "<<ptr_3_list[ptr_3_num]<<endl;
            return ptr_3_list[ptr_3_num++];
        }
    };
    double* allocate_9(){
//        cout<<"=== allocate_9 ===   ptr_9_num="<<ptr_9_num<<endl;
        if (ptr_9_num == ptr_num - 1){
            double* ptr = new double[9];
//            cout<<"=== new9 [] "<<ptr<<endl;
            return ptr;
        }
        else{
//            cout<<"=== allocate9 [] "<<ptr_9_list[ptr_9_num]<<endl;
            return ptr_9_list[ptr_9_num++];
        }
    };

    void recycle_3(double* ptr){
//        cout<<"--- recycle_3 ---   ptr_3_num="<<ptr_3_num<<endl;
        if (ptr_3_num == 0){
//            cout<<"--- delete3 [] "<<ptr<<endl;
            delete [] ptr;
        }
        else{
            ptr_3_num--;
//            cout<<"--- recycle3 [] "<<ptr<<endl;
            ptr_3_list[ptr_3_num] = ptr;
        }
//        cout<<"--- recycle_3 finished ---"<<endl;
    };
    void recycle_9(double* ptr){
//        cout<<"--- recycle_9 ---   ptr_9_num="<<ptr_9_num<<endl;
        if (ptr_9_num == 0){
//            cout<<"--- delete9 [] "<<ptr<<endl;
            delete [] ptr;
        }
        else{
            ptr_9_num--;
//            cout<<"--- recycle9 [] "<<ptr<<endl;
            ptr_9_list[ptr_9_num] = ptr;
        }
//        cout<<"--- recycle_9 finished ---"<<endl;
    };

};


MemoryAllocator allo;
...
//	cout<<" >>> constructing >>>\n";
	...
	if (num_row==3 && num_col==1){
        pbody = allo.allocate_3();
    }
    else{
        if (num_row==3 && num_col==3){
            pbody = allo.allocate_9();
        }
    }
...
//    }
    if (num_row==3 && num_col==1){
        allo.recycle_3(pbody);
    }
    else{
        if (num_row==3 && num_col==3){
            allo.recycle_9(pbody);
        }
    }
...
void Matrix::dimension(int row,int col)
{
	...
	if(pbody!=0){cerr<<"Warning: Dimension Change !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!' ***\n";exit(1);}
...
//    std::cout<<num_row<<"x"<<num_col<<" "<<B.num_row<<"x"<<B.num_col<<endl;
...
Matrix Matrix::univec3()
{
	...
	//check for proper dimensions
	if (num_col!=1||num_row!=3)
	{cout<<"*** Error: not a 3x1 column vector 'Matrix::univec()' *** ";exit(1);}
...
double Datadeck::look_up(string name,double value1)
{
	...
	auto temp_table = table_map[name];
	int var1_dim=temp_table->get_var1_dim();
	int loc1=find_index(var1_dim-1,value1,temp_table->var1_values);

	//using max discrete value if value is outside table
	if (loc1==(var1_dim-1)) return temp_table->data[loc1];

	return interpolate(loc1,loc1+1,name,value1);
}
...
double Datadeck::look_up(string name,double value1,double value2)
{
	...
	int var2_dim=temp_table->get_var2_dim();
	int loc2=find_index(var2_dim-1,value2,temp_table->var2_values);

	return interpolate(loc1,loc1+1,loc2,loc2+1,name,value1,value2);
}
...
double Datadeck::look_up(string name,double value1,double value2,double value3)
{
	...
	int var3_dim=temp_table->get_var3_dim();
	int loc3=find_index(var3_dim-1,value3,temp_table->var3_values);

	return interpolate(loc1,loc1+1,loc2,loc2+1,loc3,loc3+1,name,value1,value2,value3);
}
///////////////////////////////////////////////////////////////////////////////
//Table index finder
...
double Datadeck::interpolate(int ind1,int ind2,string &name,double val)
{
	// 20221123 使用temp_table减少引索次数
	// 20221123 函数内逻辑优化
	double dx(0);
	auto temp_table = table_map[name];
	dx=temp_table->var1_values[ind2]-temp_table->var1_values[ind1];

	if(dx>EPS){
        return temp_table->data[ind1] + (temp_table->data[ind2]-temp_table->data[ind1]) * (val-temp_table->var1_values[ind1]) / dx;
	}
	else{
        return temp_table->data[ind1];
	}
}
///////////////////////////////////////////////////////////////////////////////
...
double Datadeck::interpolate(int ind10,int ind11,int ind20,int ind21,string &name,double value1,
					double value2)
{
	// 20221123 使用temp_table减少引索次数
	// 20221123 函数内逻辑优化
	...
	int var1_dim=temp_table->get_var1_dim();
	int var2_dim=temp_table->get_var2_dim();
	bool flag1=(ind10==(var1_dim-1));
	bool flag2=(ind20==(var2_dim-1));

    if(!flag1 && !flag2){
        double diff1=value1-temp_table->var1_values[ind10];
	    double diff2=value2-temp_table->var2_values[ind20];
    	dx1=temp_table->var1_values[ind11]-temp_table->var1_values[ind10];
        dx2=temp_table->var2_values[ind21]-temp_table->var2_values[ind20];
        if(dx1>EPS) dumx1=diff1/dx1;
	    if(dx2>EPS) dumx2=diff2/dx2;
        double y11=temp_table->data[ind10*var2_dim+ind20];
	    double y12=temp_table->data[ind10*var2_dim+ind21];
	    double y21=temp_table->data[ind11*var2_dim+ind20];
	    double y22=temp_table->data[ind11*var2_dim+ind21];
	...
	if(flag1 && !flag2){
	    ...
	    dx2=temp_table->var2_values[ind21]-temp_table->var2_values[ind20];
	    if(dx2>EPS) dumx2=diff2/dx2;
	    double y1=temp_table->data[ind10*var2_dim+ind20];
	    double y2=temp_table->data[ind10*var2_dim+ind21];
	    return dumx2*(y2-y1)+y1;
	}
	if(flag1 && flag2){
	    return temp_table->data[ind10*var2_dim+ind20];
	}
...
double Datadeck::interpolate(int ind10,int ind11,int ind20,int ind21,int ind30,int ind31,
							 string &name,double value1,double value2,double value3)
{
	auto temp_table = table_map[name];
	...
	double diff1=value1-temp_table->var1_values[ind10];
	double diff2=value2-temp_table->var2_values[ind20];
	double diff3=value3-temp_table->var3_values[ind30];
	...
	if(ind30==(var3_dim-1)) //Assures constant upper extrapolation of third variable
		...
		dx3=temp_table->var3_values[ind31]-temp_table->var3_values[ind30];
	...
	// For parameter ind20
	double y11=temp_table->data[ind10*var2_dim*var3_dim+ind20*var3_dim+ind30];
	double y12=temp_table->data[ind10*var2_dim*var3_dim+ind20*var3_dim+ind30+var2_dim*var3_dim];
	double y31=temp_table->data[ind10*var2_dim*var3_dim+ind20*var3_dim+ind31];
	double y32=temp_table->data[ind10*var2_dim*var3_dim+ind20*var3_dim+ind31+var2_dim*var3_dim];
	//2DIM interpolation
	...
	// For parameter ind21
	y11=temp_table->data[ind10*var2_dim*var3_dim+ind21*var3_dim+ind30];
	y12=temp_table->data[ind10*var2_dim*var3_dim+ind21*var3_dim+ind30+var2_dim*var3_dim];
	y31=temp_table->data[ind10*var2_dim*var3_dim+ind21*var3_dim+ind31];
	y32=temp_table->data[ind10*var2_dim*var3_dim+ind21*var3_dim+ind31+var2_dim*var3_dim];
	//2DIM interpolation
...
{
	return y+(dydx_new+dydx)*int_step*0.5;
}
///////////////////////////////////////////////////////////////////////////////
